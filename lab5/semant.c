...
/* my code */
int err_open = 0;
void errlog(int pos,char* s,int i){ if(err_open)EM_error(pos,"%s %d",s,i);}
void errlog2(int pos,char* s,string str){ if(err_open)EM_error(pos,"%s %s",s,str);}
...
Ty_tyList recu_tlist(S_table tenv, A_fieldList params) {
    if (params == NULL) {
        return NULL;
    }
    Ty_ty type = S_look(tenv, params->head->typ);
    return Ty_TyList(type, recu_tlist(tenv, params->tail));
}

U_boolList recu_escape(A_fieldList params) {
    if (params == NULL) {
        return NULL;
    }
    return U_BoolList(params->head->escape, recu_escape(params->tail));
}

Ty_fieldList recu_flist(S_table tenv, A_fieldList fields) {
...
struct expty transVar(S_table venv, S_table tenv, A_var v, Tr_level level, Temp_label nearest_break) {
    ...
    switch (v->kind) {
        case A_simpleVar: {
            ...
            // get IR node
            Tr_exp tr_exp = Tr_simpleVar(var_entry->u.var.access, level);

            return expTy(tr_exp, actual_ty(var_entry->u.var.ty));
        }
        case A_fieldVar: {
            // check left value
            struct expty lvalue = transVar(venv, tenv, v->u.field.var, level, nearest_break);
        ...
        case A_subscriptVar: {
            ...
            struct expty lvalue = transVar(venv, tenv, v->u.subscript.var, level, nearest_break);
    ...
    switch (a->kind) {
        ...
        case A_callExp: {
            ...
            for (; args && expects; args = args->tail, expects = expects->tail) {
                // translate parameter
                struct expty var_entry = transExp(venv, tenv, args->head, level, nearest_break);
            ...
            list = list->tail;
            Tr_exp tr_exp = Tr_call(fun_entry->u.fun.label, list, level, fun_entry->u.fun.level);
        ...
        case A_opExp: {
            ...
            } else {  // other ops just need the same operants' type
        ...
        case A_recordExp: {
            ...
            for (; actuals && expects; actuals = actuals->tail, expects = expects->tail) {
                ...
                // field type
                struct expty var_entry = transExp(venv, tenv, actuals->head->exp, level, nearest_break);
        ...
        case A_seqExp: {
            ...
            while (exps) {
                struct expty exp = transExp(venv, tenv, exps->head, level, nearest_break);
        ...
        case A_assignExp: {
			...
			while (v->kind!=A_simpleVar){
				if (v->kind==A_fieldVar)
					v = v->u.field.var;
				if (v->kind==A_subscriptVar)
					v = v->u.subscript.var;
			}
			if (((E_enventry)S_look(venv,v->u.simple))->readonly){
        ...
        case A_ifExp: {
            ...
            if (a->u.iff.elsee) {  // if-then-else
                struct expty elsee = transExp(venv, tenv, a->u.iff.elsee, level, nearest_break);
                ...
                if (then_type != else_type && !(then_type->kind == Ty_record && else_type->kind == Ty_nil)) {
					...
					EM_error(a->u.iff.then->pos,"%d %d",then_type->kind,else_type->kind);
                }

                tr_exp = Tr_ifthenelse(test.exp, then.exp, elsee.exp);
            } else {  // just if-then
                if (actual_ty(then.ty) != Ty_Void()) {
                    EM_error(a->u.iff.then->pos, "if-then exp's body must produce no value %d",actual_ty(then.ty));
                }
        ...
        case A_letExp: {
            ...
            struct expty exp = transExp(venv, tenv, a->u.let.body, level, nearest_break);
            S_endScope(tenv);
            S_endScope(venv);
...
void transDec(S_table venv, S_table tenv, A_dec d, Tr_level level, Temp_label nearest_break) {
	errlog(d->pos,"transdec",d->kind);
    switch (d->kind) {
        case A_varDec: {
			errlog2(d->pos,"vardec:",S_name(d->u.var.var));
            // translate init data
            struct expty init = transExp(venv, tenv, d->u.var.init, level, nearest_break);

            // check variable type
            if (d->u.var.typ) {  // if type is specified
                ...
                S_enter(venv, d->u.var.var, E_VarEntry(Tr_allocLocal(level, FALSE), type));
            } else {  // otherwise, infer type from init data
                if (actual_ty(init.ty)->kind == Ty_nil) {  // can not be nil then
                    EM_error(d->u.var.init->pos, "init should not be nil without type specified");
                    break;
                }

                S_enter(venv, d->u.var.var, E_VarEntry(Tr_allocLocal(level, FALSE), init.ty));
            }

            break;
        }
        case A_typeDec: {
			errlog2(d->pos,"typedec:",S_name(d->u.type->head->name));
        ...
        case A_functionDec: {
			errlog2(d->pos,"funcdec:",S_name(d->u.function->head->name));
            // check if there are same type names in a batch declarations
            TAB_table name_table = TAB_empty();
            ...
            while (funcs) {
                ...
                U_boolList formal_escapes = recu_escape(funcs->head->params);
                Tr_level new_level = Tr_newLevel(level, func_label, formal_escapes);

                // add declaration
                Ty_tyList formal_tys = recu_tlist(tenv, funcs->head->params);
...
Ty_ty transTy(S_table tenv, A_ty a) {
    ...
    switch (a->kind) {
        ...
        case A_recordTy:
            return Ty_Record(recu_flist(tenv, a->u.record));
        case A_arrayTy: {
