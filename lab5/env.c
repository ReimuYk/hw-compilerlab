...
S_table E_base_venv(void)
{
	...
	formals->head = Ty_Int();
	formals->tail = NULL;
	S_enter(venv,S_Symbol("exit"),E_FunEntry(level,label,formals,Ty_Int()));
	S_enter(venv,S_Symbol("not"),E_FunEntry(level,label,formals,Ty_Int()));
	S_enter(venv,S_Symbol("chr"),E_FunEntry(level,label,formals,Ty_String()));
	S_enter(venv,S_Symbol("getchar"),E_FunEntry(level,label,NULL,Ty_String()));

	formals = checked_malloc(sizeof(*formals));
	formals->head = Ty_String();
	formals->tail = NULL;
	S_enter(venv,S_Symbol("print"),E_FunEntry(level,label,formals,Ty_Void()));
	S_enter(venv,S_Symbol("ord"),E_FunEntry(level,label,formals,Ty_Int()));
	S_enter(venv,S_Symbol("size"),E_FunEntry(level,label,formals,Ty_Int()));

	formals = checked_malloc(sizeof(*formals));
	formals->head = Ty_String();
	formals->tail = checked_malloc(sizeof(*formals));
	formals->tail->head = Ty_String();
	S_enter(venv,S_Symbol("concat"),E_FunEntry(level,label,formals,Ty_String()));
	...
	S_enter(venv,S_Symbol("substring"),E_FunEntry(level,label,formals,Ty_String()));
