	...
	switch(d->kind){
		...
		case A_functionDec:{
			...
			for(funcs=get_funcdec_list(d); funcs; funcs=funcs->tail){
				...
				Ty_ty res_ty = result ? S_look(tenv, result) : Ty_Void();
				S_enter(venv, name, E_FunEntry(newl, fname, formals, res_ty));
			}
			...
			for(funcs=get_funcdec_list(d); funcs; funcs=funcs->tail){
				...
				A_fieldList pl;
				Tr_accessList acc;
				for(pl=params, acc=Tr_formals(newl)->tail; pl; pl=pl->tail, acc=acc->tail){
					A_field param = pl->head;
					S_symbol name = param->name; 
					S_symbol typ = param->typ;
...
struct expty transExp(S_table venv, S_table tenv, A_exp a, Tr_level l, Temp_label label){
	switch(a->kind){
		...
		case A_intExp: {
			int i = a->u.intt;
			return expTy(Tr_int(i), Ty_Int());
		}
		case A_stringExp: {
			string str = a->u.stringg;
			return expTy(Tr_string(str), Ty_String());
		}
		case A_callExp:{
			...
			if(x && x->kind == E_funEntry){
				...
				return expTy(Tr_call(fname, ls, flev, l, S_name(func)), result);
			}
			else{
	    ...
	    case A_opExp:{
			...
			switch(oper){
				...
				case A_plusOp:case A_minusOp:case A_timesOp:case A_divideOp:
				{
					...
					return expTy(Tr_arithmetic(oper,le.exp,r.exp), Ty_Int());
				}
				//compare
				default:
				{
					if(actualTy(le.ty)->kind == actualTy(r.ty)->kind || le.ty->kind == Ty_nil || r.ty->kind == Ty_nil){
						if(actualTy(le.ty)->kind == Ty_string){
							return expTy(Tr_cond_str(oper,le.exp,r.exp), Ty_Int());
						}
						else{
							return expTy(Tr_cond_norm(oper,le.exp,r.exp), Ty_Int());
						}
					} 					
					else{
		...
		case A_recordExp: {
			...
			if(typp && typp->kind == Ty_record){
				...
				return expTy(Tr_record(el, cnt), type);
			}
			else{
		...
		case A_ifExp:{
			...
			if(actualTy(elsety.ty)->kind == Ty_nil){
				if(actualTy(thenty.ty)->kind == Ty_void){
					return expTy(Tr_if(testty.exp,thenty.exp,elsety.exp, thenty.ty), thenty.ty);
				}
				else{
					return expTy(Tr_if(testty.exp,thenty.exp,elsety.exp, thenty.ty), thenty.ty);
				}
				...
				if(actualTy(thenty.ty)->kind == actualTy(elsety.ty)->kind){
					return expTy(Tr_if(testty.exp,thenty.exp,elsety.exp, elsety.ty), elsety.ty);
				}
				else{
	    ...
	    case A_whileExp:{
			...
			if(bodyty.ty->kind == Ty_void){
				return expTy(Tr_while(testty.exp, bodyty.exp, done), bodyty.ty);
			}
			else{
		...
		case A_forExp:{
			...
			if(loty.ty->kind == Ty_int && loty.ty->kind == hity.ty->kind){
				...
				if(bodyty.ty->kind == Ty_void){
					...
					return expTy(Tr_for(forv, loty.exp, hity.exp, bodyty.exp, done), bodyty.ty);
				}
				else{
		...
		case A_letExp:{
			...
			return expTy(Tr_let(li, bodyty.exp),bodyty.ty);
		...
		case A_arrayExp:{
			...
			if(typp && typp->kind == Ty_array){
				...
				return expTy(Tr_array(sizety.exp, initty.exp), type);
			}
			else{
				EM_error(a->pos, "undefined type %s", S_name(typ));
				return expTy(Tr_err(), Ty_Int());
